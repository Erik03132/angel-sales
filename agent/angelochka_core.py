...
def detect_role(query: str, sender_id: str = None, sender_name: str = None) -> str:
    ...
    try:
        ...
        _roles_path = os.path.join(_agent_dir, "roles_config.json")
        # Защита от path traversal: проверяем что realpath внутри директории агента
        _roles_real = os.path.realpath(_roles_path)
        if not _roles_real.startswith(os.path.realpath(_agent_dir) + os.sep):
            print(f"⚠️ detect_role: path traversal blocked: {_roles_real}")
            return ROLE_EMPLOYEE
        if os.path.exists(_roles_real):
            with open(_roles_real, "r", encoding="utf-8") as _f:
                _cfg = json.load(_f)
        ...
        # Fail-closed: битый конфиг → минимальные права (не ROLE_CUSTOMER!)
...
_TIER_MODELS = {
    ...
    "std": [
        ...
        "moonshotai/kimi-k2.6",                       # $0.73/$3.49 — 262K, крайний резерв
    ],
    "pro": [
        "deepseek/deepseek-v4-pro",                   # $0.43/$0.87 — 1048K, умная
        "moonshotai/kimi-k2.6",                       # $0.73/$3.49 — фоллбэк
...
def _call_openrouter(prompt, history=None, system_prompt=None, tier=None):
    """Вызов через OpenRouter с маршрутизацией по сложности и prompt caching."""
...
📖 ЭТАЛОННЫЕ ОТВЕТЫ НА ТИПОВЫЕ ВОПРОСЫ (ИМИТИРУЙ ЭТИ ПАТТЕРНЫ):
ВОПРОС: Какие сейчас цены на бройлеров?
ОТВЕТ-ЭТАЛОН: «Андрей, актуальные цены по каталогу: бройлеры от 180₽/кг. Сейчас в наличии 150 голов весом 2-2.5кг. Нужно уточнить у поставщика завоз — могу запросить.»

ВОПРОС: Сколько сделок за месяц?
ОТВЕТ-ЭТАЛОН: «За текущий месяц: [ДАННЫЕ ИЗ CRM]. Конверсия: [ПРОЦЕНТ]. Если нужно подробнее — могу разбить по менеджерам.»

ВОПРОС: Какие породы сейчас в наличии?
ОТВЕТ-ЭТАЛОН: «Сейчас в наличии: [СПИСОК ИЗ КАТАЛОГА]. Самые ходовые — бройлеры и несушки. Завоз планируется на [ДАТА].»

ВОПРОС: Как дела с доставкой?
ОТВЕР-ЭТАЛОН: «Доставка работает по ПН и ЧТ. Основные направления — Крым и Юг России. За последние 7 дней: [ДАННЫЕ ИЗ CRM].»

ВОПРОС: Что с менеджерами?
ОТВЕТ-ЭТАЛОН: «Менеджеры: [ДАННЫЕ ИЗ CRM]. Кто-то выделяется по продажам, у кого-то есть проблемы. Могу подробнее по каждому.»
...
def build_system_prompt(role, db_context="", vector_context="", history=None):
    ...
    if role == ROLE_CREATOR:
        ...
        ВОПРОС: Как дела с доставкой?
        ОТВЕТ-ЭТАЛОН: «Доставка работает по ПН и ЧТ. Основные направления — Крым и Юг России. За последние 7 дней: [ДАННЫЕ ИЗ CRM].»
        ...
        📖 ЭТАЛОННЫЕ ОТВЕТЫ НА ТИПОВЫЕ ВОПРОСЫ (ИМИТИРУЙ ЭТИ ПАТТЕРНЫ):
        ВОПРОС: Сколько стоит [порода]?
        ОТВЕТ-ЭТАЛОН: «По каталогу: [порода] от [цена]₽/кг. Сейчас в наличии [кол-во] голов. Нужно больше — могу уточнить у поставщика.»
        
        ВОПРОС: Какой корм давать [породе]?
        ОТВЕТ-ЭТАЛОН: «Для [породы] рекомендую: [тип корма]. Норма — [кол-во] г/день. Подробнее в руководстве — могу прислать ссылку.»
        
        ВОПРОС: Клиент спрашивает про доставку — что отвечать?
        ОТВЕТ-ЭТАЛОН: «Доставка: ПН и ЧТ, Крым и Юг России. Стоимость — от [суммы]₽ в зависимости от расстояния. Точный расчет — после оформления заказа.»
        
        ВОПРОС: Как рассчитать корм для стаи?
        ОТВЕТ-ЭТАЛОН: «Формула: [кол-во] голов × [норма] г/день = [итого] кг/день. На месяц нужно: [итого] × 30 = [итого] кг. Комбикорм ПК-5-1 — от [цены]₽/кг.»
        
        ВОПРОС: [ЛЮБОЙ ВОПРОС НЕ ПО ТЕМЕ ПТИЦЫ]
        ОТВЕТ-ЭТАЛОН: «Коллега, это не моя тема. Я помогаю с птицей, ценами и доставкой. По другим вопросам — лучше к руководству.»
