#!/usr/bin/env python3
"""Leonardo.ai — генерация 16 фото (ИСПРАВЛЕНО: generations_by_pk)"""
...
print("="*70)
print("🎨 Leonardo.ai Phoenix — 16 фото (ИСПРАВЛЕНО)")
print("="*70)
print()
...
for folder, prompt in folders_prompts:
    ...
    try:
        ...
        # Polling (ПРАВИЛЬНЫЙ ПАРСИНГ: generations_by_pk)
        for i in range(20):
            time.sleep(4)
            status = session.get(f"{BASE_URL}/generations/{gen_id}", headers=headers, timeout=30).json()
            
            # ИСПРАВЛЕНО: используем generations_by_pk
